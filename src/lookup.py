...
from config.feature import street_names_path, postcodes_path
...
level_types = ('BASEMENT', 'FLOOR', 'GROUND', 'LEVEL', 'LOBBY', 'LOWER GROUND FLOOR', 'MEZZANINE', 'OBSERVATION DECK',
               'PARKING', 'PENTHOUSE', 'PLATFORM', 'PODIUM', 'ROOFTOP', 'SUB-BASEMENT', 'UPPER GROUND FLOOR', 'GROUND FLOOR')
...
street_suffix_types = dict([('CN', 'CENTRAL'), ('DE', 'DEVIATION'), ('E', 'EAST'), ('EX', 'EXTENSION'),
...
street_names = dict(pd.read_csv(street_names_path, sep='\t', header=0).values)

post_codes = pd.read_csv(postcodes_path, 
                         sep='\t', 
                         header=0, 
                         names=['country_code', 'postal_code', 'place_name', 'state', 'state_code', 'county', 'county_code', 'community', 'community_code', 'latitude', 'longitude', 'accuracy']
            ).to_dict('records')
...
def _lookup(t: str, types: List[str]) -> int:
    """
...
def _reverse_lookup(idx: int, types: List[str]) -> str:
    """
...
def abbreviate_street_type(street_type: str) -> str:
    """
    Converts a full street type to the abbreviated name, e.g. "STREET" -> "ST"
